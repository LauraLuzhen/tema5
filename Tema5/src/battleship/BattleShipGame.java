...
public class BattleShipGame {
	 ...
	 *         otherwise false
	 */
	private boolean checkPosition(int row, int column, char table[][]) {
		boolean placed = true;

		if (row >= 0 && row < machineTable.length && column >= 0 && column < machineTable.length) {
			// Right row
			if (column + 1 < machineTable.length) {
				if (table[row][column + 1] == ship) {
					placed = false;
				}
				if (row + 1 < machineTable.length) {
					if (table[row + 1][column + 1] == ship) {
						placed = false;
					}
				}
				if (row - 1 >= 0) {
	...
	public boolean movePlayerShips(char rowShip, int columnShip, char[][] table) {
		...
		return checkPosition(row, column, table);
	}
	...
	public void moveMachineShips() {
		...
		while (count < NUM_SHIPS) {
			...
			if (checkPosition(row, column, machineTableShips)) {
				count++;
			}
		}
	}
